...
describe('Quiz Question Create tests', () => {
  ...
  describe('Success Cases', () => {
    test('Correct return value', () => {
      const questionCreateResult = adminQuizQuestionCreateV2(
        quizId,
        sessionId,
        VALID_QUESTION
      );
      ...
      test.each([
        ...
        ({ points, timeLimit }) => {
          ...
          const questionId = (questionCreateResult as { questionId: number })
            .questionId;
          const quizInfoResult = adminQuizInfoV2(sessionId, quizId);
    ...
    test('Multiple questions creation', () => {
      const questionCreateResult1 = adminQuizQuestionCreateV2(
        quizId,
        sessionId,
        VALID_QUESTION
      );
      ...
      const questionCreateResult2 = adminQuizQuestionCreateV2(
        quizId,
        sessionId,
        VALID_QUESTION_2
      );
  ...
  describe('Error Cases', () => {
    describe('Invalid Session', () => {
      test.each([
        ...
        const questionCreateResult = adminQuizQuestionCreateV2(quizId, session, {
          question: 'What is the capital of France?',
          timeLimit: TIME_LIMITS.VALID,
          points: POINTS_LIMITS.VALID,
          answerOptions: [
    ...
    describe('Unauthorised Access', () => {
      ...
      test('Valid session, but quiz does not exist', () => {
        const questionCreateResult = adminQuizQuestionCreateV2(9999, sessionId, {
          question: 'What is the capital of France?',
          timeLimit: TIME_LIMITS.VALID,
          points: POINTS_LIMITS.VALID,
          answerOptions: [
    ...
    describe('Invalid Time Limits', () => {
      ...
      test(`Sum of time limits exceeds ${TIME_LIMITS.TOTAL_MAX} seconds`, () => {
        adminQuizQuestionCreateV2(quizId, sessionId, {
          question: 'Question 1',
          timeLimit: TIME_LIMITS.VALID,
          points: POINTS_LIMITS.VALID,
          answerOptions: [
    ...
    describe('Invalid ThumbnailUrl', () => {
      test.each([
        ...
        },
        {
          ...
          description: 'invalid file extension (not jpg, jpeg, or png)',
        },
        {
          thumbnailUrl: 'example.com/image.jpg',
          description: 'does not begin with http:// or https://',
        },
        {
          thumbnailUrl: 'file:///image.jpg',
          description: 'does not begin with http:// or https://',
        },
      ])('ThumbnailUrl is $description', ({ thumbnailUrl }) => {
        const questionCreateResult = adminQuizQuestionCreateV2(
          quizId,
          sessionId,
          {
