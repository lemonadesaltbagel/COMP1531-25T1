...
describe('adminQuizTransfer', () => {
  ...
  describe('adminQuizTransferV2 - Error Cases', () => {
    test('Fails for invalid session', () => {
      response = adminQuizTransferV2(
        quizId,
        sessionId + 'error',
        secondValidEmail
      );
    ...
    test('Fails when user does not own the quiz', () => {
      ...
      const result = adminQuizTransferV2(
        secondQuizId,
        sessionId,
        secondValidEmail
      );
    ...
    test('Fails for invalid Email', () => {
      response = adminQuizTransferV2(quizId, sessionId, invalidEmail);
      expect(response).toEqual(HTTP_STATUS.INVALID_EMAIL);
    });

    test('Fails for invalid Email that matches sessionId user', () => {
      response = adminQuizTransferV2(quizId, sessionId, validEmail);
    ...
    test('Fails for using name of already created quiz owned by logged in user', () => {
      ...
      response = adminQuizTransferV2(quizId, sessionId, secondValidEmail);
      expect(response).toEqual(HTTP_STATUS.BAD_REQUEST);
    });

    test('Game for this quiz is not in END state', () => {
      ...
      const result = adminQuizTransferV2(quizId, sessionId, secondValidEmail);
      expect(result).toStrictEqual(HTTP_STATUS.BAD_REQUEST);
    });
  });

  describe('adminQuizTransferV2 - Success Cases', () => {
    ...
    test('Successfully returns quizId for multiple quizzes', () => {
      ...
      response = adminQuizTransferV2(secondQuizId, sessionId, secondValidEmail);
      expect(response).toStrictEqual({});
